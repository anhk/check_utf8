...
static int check_utf8_str(lua_State *L)
{
	int len = luaL_checknumber(L, 2);
	const char *str = luaL_checkstring(L, 1);
	int ret = check_utf8(str, len);
	if (ret == 0) {
		lua_pushboolean(L, 1);
	} else {
		lua_pushboolean(L, 0);
...
static const struct luaL_Reg utf8_reg [] = {
	{"check_utf8", check_utf8_str}, 
	{NULL, NULL}
};

int luaopen_utf8(lua_State *L)
{
	luaL_openlib(L, "utf8", utf8_reg, 0);
	return 1;
}

int luaopen_util_utf8(lua_State *L)
{
